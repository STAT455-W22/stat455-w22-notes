...
* Assuming we're comparing households in the same region, for each y-year increase in age of head of household, average household size is expected to multiply be a factor of $e^{-0.004598y}$.   
    - For example expected household size for a 50 year old head of house is expected to be $e^{-.004598*10}$ times that of a 40 year old head of house (an estimated 4.5% decrease).    
...
When the null hypothesis is true, the model's residual deviance follows a $\cha^2$ distribution with $n-p$ degrees of freedom, where $p$ is number of parameters in the model.  
...
We can estimate a dispersion parameter, $\hat\phi=\frac{\sum(\textrm{Pearson residuals})^2}{n-p}$, where $p$ is the number of model parameters. 
