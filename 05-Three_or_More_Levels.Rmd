...
```{r}
Fish <- 1:24
Tank <- c(rep(1, 4), rep(2, 4), rep(3, 4), rep(4, 4), rep(5, 4), rep(6, 4))
...
Activity <- c(55, 45, 45, 55, 40, 50, 30, 60, 80, 40, 45, 75, 50, 66, 45, 71, 75, 35, 60, 50, 40, 70, 60, 50)
...
d) Output for the linear mixed effect model in (c) is given below. Use the output to answer the following questions. 

```{r, eval=FALSE}
M <- lmer(data=FishData, Activity ~ Temp + (1|Tank) + (1|Aquarium))
summary(M)
```
