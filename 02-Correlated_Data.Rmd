...
\[
\frac{\bar{x}_1-\bar{x}_2}{\sqrt{\frac{s_1^2}{n_1}+\frac{s_1^2}{n_1}}}
\]
...
|:------|:---------------|:--------------|
| A | $\beta_0$ | $\epsilon_{ij}$|   
| B | $\beta_0 + \beta_1$ |  $\epsilon_{ij}$ |
| C | $\beta_0 + \beta_2$ | $\epsilon_{ij}$ |
...
Litter| Diet|  Expected Weight | Random Deviation | 
|:----|:------|:---------------|:--------------|
| 1 | A |  $\beta_0$ | $l_1 + \epsilon_{1j}$|   
| 1 | B |  $\beta_0 + \beta_1$ | $l_1 + \epsilon_{1j}$|   
| 1 | C |  $\beta_0 + \beta_2$ | $l_1 + \epsilon_{ij}$|   
| 2 | A |  $\beta_0$ | $l_2 + \epsilon_{2j}$|   
| 2 | B |  $\beta_0 + \beta_1$ | $l_2 + \epsilon_{2j}$|   
| 2 | C |  $\beta_0 + \beta_2$ | $l_2 + \epsilon_{2j}$|   
| 3 | A |  $\beta_0$ | $l_3 + \epsilon_{1j}$|   
| 3 | B |  $\beta_0 + \beta_1$ | $l_3 + \epsilon_{3j}$|   
| 3 | C |  $\beta_0 + \beta_2$ | $l_3 + \epsilon_{3j}$|   
...
There is no reason to test for differences in weight gain between the litters. Differences between these specific litters of mice are not important to us. Afterall, we are never going to see these very litters of mice!    
...
* Variables that we want to test for differences or relationships between are called **fixed effects**. We should build these into the "expectation structure" of the model, using $\beta_j$'s.   
