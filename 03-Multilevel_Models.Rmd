...
model3c <- lmer(na ~ orch + cmpqnem + large +  (1 | id), REML = T, data = music, REML=TRUE)
summary(model3c)
```
...
modelA <- lmer(na ~ previous + students + juried + 
    ...
    public + solo | id), REML = T, data = music, REML=TRUE)
summary(modelA)
```
...
modelB <- lmer(na ~ previous + students + juried + 
    public + solo + mpqpem + mpqab + orch + mpqnem + 
    mpqnem:solo + (1 | id), REML = T, data = music, REML=TRUE)
summary(modelB)
```
