...
```{r}
music <- music %>% mutate(ageover18 = age-18)
```
...
$$
\begin{align*}
Y_{ij} & = [\alpha_{0}+\alpha_{1}\textrm{Ageover18}_{i}+\beta_{0}\textrm{Juried}_{ij}+\beta_{1}\textrm{Ageover18}_{i}\textrm{Juried}_{ij}] \\
...
b. $\hat{\beta_1}$ is negative and the associated p-value is small.    
c. $\hat{\sigma} > \hat{\sigma_u}$     
d. $\hat{\sigma_u}$ is about the same as  $\hat{\sigma_v}$    
e. $\hat{\rho}_{uv}$ is negative     
...
$$
\begin{align*}
Y_{ij} & = [\alpha_{0}+\alpha_{1}\textrm{Ageover18}_{i}+\beta_{0}\textrm{Juried}_{ij}] \\
 & \textrm{} + [u_{i}+v_{i}\textrm{Juried}_{ij}+\epsilon_{ij}]
\end{align*}
$$
