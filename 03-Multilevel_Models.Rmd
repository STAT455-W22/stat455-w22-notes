...
We examine the  distribution of level 2 covariates (obtained from first performance by each musician, since these will be the same for all performances). 
...
### Lattice Plot for Previous Performances vs Nagative Affect
...
We create variables for these characteristics. 
...
where $u_i \sim\mathcal{N}(0,\sigma^2)$, and $\epsilon_{ij} \sim\mathcal{N}(0,\sigma^2)$. We assume $u_i$ and $\epsilon_{ij}$ are independent. 
...
```{r, comment=NA}
model1 <- lmer(data=music, na ~ orch + large + orch:large + (1 | id))
summary(model1)
```
...
* After accounting for performance type and instrument type, and their interaction, there the standard deviation in negative affect scores between different musicians is estimated to be  $\hat{\sigma}_u=2.265$.    

* After accounting for performance type, instrument type, and their interaction, there the standard deviation in negative affect scores between different performances by the same musician is estimated to be  $\hat{\sigma}_u=4.68$.    
...
```{r, comment=NA}
model1b <- lmer(data=music, na ~ orch + large  + (1 | id))
summary(model1b)
```
...
* Expected negative affect differs between instrument types and performance types, and the effect of performing in a large ensemble is allowed to differ between instrumental and non-instrumental musicians.     
...
To do this, we add a random effect for the slope (or in this case difference) between performance types for each performer.  
...
```{r, comment=NA}
model2b <- lmer(data=music, na ~ orch + large  + (large | id))
summary(model2b)
```
...
Both $\beta=-1.74$ and $\rho_{uv}=-0.59$ seem to suggest negative relationships involving performance anxiety associated with playing in a large ensemble.

Let's think carefully about what each of these tells us, and how they're different.    

* $\beta=-1.74$ tells us that on average, a musicians's negative affect score for performance anxiety is expected to decrease by 1.74 points when playing in a large ensemble, compared to a solo or small ensemble performance.    

* $rho_uv = -0.59$ tells us that musicians who have higher performance anxiety than expected when playing a solo or small ensemble tend to see a bigger decrease in anxiety when playing in a large ensemble than those who have less anxiety playing in a solo or small ensemble.  

**Illustration:**

Each line represents one of the 37 musicians. The thick black line represents average change.   

```{r,echo=FALSE, warning=FALSE}

model2c <- lmer(data=music, na ~ large + orch  + (large | id))
...
```{r, comment=NA}
model2 <- lmer(data=music, na ~ orch + large + orch:large + (large | id))
summary(model2)
```


### Formulation of Random Slopes Model with Interaction

Two level model for performance anxiety, using large ensemble and playing an orchestral instrument as explanatory variables. 
...
**4 fixed effects:** $\alpha_{0}$, $\alpha_{1}$, $\beta_{0}$ and $\beta_{1}$. 

* Fixed effects are the fixed but unknown population effects associated with certain covariates. 

* The intercepts and slopes for each subject from Level One, $a_{i}$ and $b_{i}$, don't need to be formally estimated. They serve to conceptually connect Level One with Level Two. 

Substituting the two Level Two equations into the Level One equation gives: 
...
model3 <- lmer(na ~ orch + mpqnem + large  + (1 | id), data = music)
summary(model3)
```
...
model3c <- lmer(na ~ orch + cmpqnem + large +  (1 | id), REML = T, data = music)
summary(model3c)
```
...
Since model C is a nested version of model D, we use a **likelihood ratio test** \index{likelihood ratio test (LRT)} (drop in deviance test), to compare the models. 

When using mixed effects models, the test statistic follows a $\chi^2$ distribution, rather than an F-distribution, so we use `test = "Chisq"`. 

```{r comment=NA, message=FALSE}
# anova() automatically uses ML for LRT tests
drop_in_dev <- anova(model3, model1b, test = "Chisq")
drop_in_dev
```

There is evidence that adding negative emotionality helps better explain variability in anxiety, as measured by negative affect. 


### A Potential Final Model
