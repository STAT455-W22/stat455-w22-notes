...
Thus, we can think of a multilevel model as a model where the slope and intercept at level one are themselves linear functions of a variable at level 2.   
...
modelB <- lmer(na ~ previous + students + juried + 
    public + solo + mpqpem + mpqab + orch + mpqnem + 
    mpqnem:solo + (1 | id), REML = T, data = music)
summary(modelB)
```
