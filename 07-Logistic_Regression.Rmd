...
In Stat 255, we learned about Logistic regression models for binary response data. Please review sections 8.1-8.3 in the [Stat 255 notes](https://bookdown.org/ajsage/statistics_for_data_science_notes/) prior to proceeding. 
...
\[\frac{e^{-0.9521859 + 0.0319564\times25}}{1+ e^{ (-0.9521859 + 0.0319564\times25)}}=0.4618\]
...
p2 <- ggplot(Wisconsinresid, aes(x = Unemployment, y = Resid)) +
  geom_point() +
  xlab("Unemployment Rate") + ylab("Deviance Residuals")
p3 <- ggplot(Wisconsinresid, aes(x = WhitePct, y = Resid)) +
  geom_point() +
  xlab("Percentage of White Residents") + ylab("Deviance Residuals")
p4 <- ggplot(Wisconsinresid, aes(x = COVID_DeathRate, y = Resid)) +
  geom_point() +
  xlab("Covid Deaths per 100K") + ylab("Deviance Residuals")
p5 <- ggplot(Wisconsinresid, aes(x = Income, y = Resid)) +
  geom_point() +
  xlab("Median Income") + ylab("Deviance Residuals")
...
b) If is estimated to have a 0.5 probability of winning, and another team's percentage of three point shots attempted is 4 percentage points higher, what is the second team's estimated probability of winning?    
