...
- $\widehat{\sigma}_u$= 41.9 = the standard deviation in average math scores (averaging across the tree years), for different schools (tells us on average, how much do average scores differ between schools?)      
...
* $Var(Y_{i1})$, the uncertainty (variability) around a school's score in 2008, increases as the uncertainty in intercepts (measured by $\sigma^2_u$) and slopes (measured by $\sigma^2_v$) increases, as the uncertainty around that school's linear time trend (measured by $\sigma^2$) increases, and as the covariance between intercept and slope residuals increases (measured by $\sigma_{uv}$), since if one is off, the other one is likely off as well.  
...
- correlation is estimated to decrease as years get farther apart (though differences are not dramatic in this case)   
