...
$$
\text{ROI}_{i}=\beta_{0}+\beta_{1}\textrm{YearSince1990}_{i}+\beta_{2}\textrm{Pass}_{i}+\beta_{3}\textrm{YearSince1990}_{i}\times\beta_{2}\textrm{Pass}_{i}+\epsilon_{i} \quad \textrm{where} \quad \epsilon_{i}\sim \textrm{N}(0,\sigma^2).
$$
...
a) Explain why we cannot assess the appropriateness of the normality assumption by looking only at a histogram of the response variable. For example, why shouldn't we conclude that the normality assumption in model M1 above is inappropriate, based on the fact the the histogram below is right-skewed?    

```{r}
ggplot(data=Bechdel, aes(x=totalROI)) + geom_histogram(fill="blue")
```
