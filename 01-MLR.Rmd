...
*  track `condition` (fast, good, slow)    
...
```{r resid2, fig.align = "center", out.width = "90%", fig.cap = 'Residual plots for Model 2.', echo=TRUE, warning=FALSE}
...
```{r resid2q, fig.align = "center", out.width = "90%", fig.cap = 'Residual plots for Model 2Q.', echo=TRUE, warning=FALSE}
# Residual diagnostics for Model 2
par(mar=c(1,1,1,1))
par(mfrow=c(2,2))
plot(model2q)
...
```{r model0, comment=NA}
model0A <- lm(speed ~ yearnew + yearnew2 + starters, data = derby.df)
model0B <- lm(speed ~ yearnew + yearnew2 + fast + good + starters, data = derby.df)
model0C <- lm(speed ~ yearnew + yearnew2 + fast + good + starters + yearnew:fast + yearnew:good + yearnew2:fast + yearnew2:good, data = derby.df)
```
...
* There is strong evidence that model B is better than model A. Condition of track seems to be an important factor in predicting winning speed.    
...
```{r , fig.align = "center", out.width = "90%", fig.cap = 'Residual plots for Model 0B.', echo=TRUE, warning=FALSE}
